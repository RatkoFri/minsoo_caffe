...
int test() {
  ...
  for (int i = 0; i < FLAGS_iterations; ++i) {
    ...
    if (statis_on) {
        // MINSOO Prepare batch
        batch = new Batch<float>(i, batch_size);
        for (int j = 0; j < batch_size; j++) {
            Infer<float>* infer = new Infer<float>(j,num_clayer,num_ilayer);            batch->addInfer(infer); 
        }
    }
    ...
    if (statis_on) {
        // MINSOO add batch to statis
        statis->addBatch(batch);
    }
  }
